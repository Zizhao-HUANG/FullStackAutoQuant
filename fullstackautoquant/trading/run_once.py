...
def parse_args():
    ...
    p.add_argument(
        "--current_positions",
        default=None,
        help="optional current positions json path (forwarded to strategy)",
    )
...
def main():
    ...
    gm_cmd += [
        "--max_slices_open",
        str(int(args.max_slices_open)),
        "--open_eps",
        str(float(args.open_eps)),
    ]
    if args.account_id:
    ...
    replay_cmd = [
        sys.executable,
        os.path.join(os.path.dirname(__file__), "trade_logger_replay.py"),
        "--update_nav",
    ]
    if args.config:
